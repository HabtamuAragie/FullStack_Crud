...
function Details() {
  const [getuserdata, setUserdata] = useState([]);
    ...
    const deleteuser = async (id) => {
      ...
      const res2 = await fetch(`http://localhost:8002/delete-user/${id}`, {
          method: "DELETE",
          headers: {
              "Content-Type": "application/json"
          }
  ...
  return (
    ...
    <h1 style={{ fontWeight: 400 }}>Welcome Employee Profile</h1>
    ...
    <Card sx={{ maxWidth: 600 }}>
        <CardContent>
            <div className="add_btn ">
                <Link to={`/edit/${getuserdata.id}`}>  <button className="btn btn-primary mx-2"><CreateIcon /></button></Link>
